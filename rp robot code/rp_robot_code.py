            ...
            if GPIO.input(LEFT_OUTSIDE_SENSOR) == 1 or GPIO.input(RIGHT_OUTSIDE_SENSOR) == 1:
                ...
                if GPIO.input(LEFT_OUTSIDE_SENSOR) == 1:
                    ...
                    while not (GPIO.input(LEFT_INSIDE_SENSOR) == 0 and GPIO.input(RIGHT_INSIDE_SENSOR) == 1):
                        ...
                        if GPIO.input(LEFT_OUTSIDE_SENSOR) == 1 and GPIO.input(RIGHT_OUTSIDE_SENSOR) == 1:
                            break
                        adjust_wheels_forward_speed_in_turns(33, 2)
            # if none of the outside sensors see black line
            else:
                # when the robot finishes the tour, the robot increases the forward wheels speed.
                # so he will arrive to start position faster.
                # if at_home:
                #     forward_right_speed=29
                #     forward_left_speed=30
...
def getDistance(stop_driving_cv, distance_cv):
    ...
    while not end_distance_thread_life_event.is_set():
        ...
        while GPIO.input(ECHO) == 0:
            ...
            stuck_duration = round(stuck_time_end - stuck_time_start)
        ...
        with stop_driving_cv:
            if distance < critical_distance:
                ...
                if there_is_obstacle:
                    ...
                    there_is_obstacle = False
                # check if ok if not change backwards
...
def try_to_connect_to_wifi(network_name, password):
    ...
    call("sudo python3 connect_to_wifi.py " + network_name + " " + password, shell=True)
    time.sleep(2)
...
def prepare_for_tour():
    ...
    global museum_id
    try:
        with open("museum_id.txt","r") as file:
            # Reading form a file
            museum_id = file.read()
        if museum_id == "":
            while museum_id:
                ...
                if museum_id_status == 200:
                    if museum_id_data:
                        ...
                        try:
                            with open("museum_id.txt", "w") as file:
                    ...
                    play_mp3_file("/home/pi/play_text_and_sounds/failed_reaching_the_server.mp3")
                    shutdown_rp()
    except IOError as e:
        play_mp3_file("/home/pi/play_text_and_sounds/barcode_not_valid.mp3")
...
def battery_manager():
    ...
    while True:
        if battery_remaining_usage_time <= FULL_BATTERY_CAPACITY/4 and (not battery_power_25):
            ...
            turn_gpio_pin_on_or_off(BATTERY_WARNING, PIN_ON)  # turn on left led
            PWM_BATTERY_WARNING.start(50)  # make the left led flicker
        if not low_battery_power_event.is_set():
            if battery_remaining_usage_time <= CRITICAL_BATTERY_TIME:
                ...
                # stop robot from speaking if he does
        ...
        try:
            ...
            with open("battery_remaining_usage_time.txt", "w") as file:
                # Writing data to a file
...
# check battery
    ...
    while not read_completed:
        try:
            with open("battery_remaining_usage_time.txt", "r") as file:
                # Reading form a file
                battery_remaining_usage_time = int(file.read())
            with open("time_takes_drive_from_start_to_finish.txt", "r") as file:
                # Reading form a file
...
time_of_last_played_mp3_file = 0
...
while True:
    ...
    if br_data == "yes":
        ...
        while br_data !="yes" and br_data!="no":
            if br_data== "yes":
                try:
                    with open("time_takes_drive_from_start_to_finish.txt", "w") as file:
                        # Writing data to a file
